...
\textbf{Fairness.} Per-class precision, recall, and false-negative rate are reported alongside macro-F1, since an aggregate metric can hide poor performance on rare classes. Donateacry-corpus skews toward English-speaking, Western, smartphone-equipped households, so cross-source held-out evaluation is run as a proxy for population shift. Generative augmentation is used as a fairness intervention: it is targeted at the rare classes (\textit{belly\_pain}, \textit{burping}), and its effect on those slices is reported as a primary result.
...
\textbf{Safety and Security.} A missed pain cry is the failure mode with the highest cost, so the false-negative rate on \textit{belly\_pain} is reported as a primary metric alongside calibration error. Robustness is evaluated under MUSAN babble at multiple SNRs and on cross-source held-out clips. The app does not autonomously dispense medication or trigger an alarm cascade. Notification thresholds are calibrated against the false-positive cost of waking a household at 3~a.m.

\section*{My Viewpoint}
The direct-to-consumer parent-app framing is more ethically risky than the NICU decision-support framing, even though it is more attractive commercially. A new parent at 3~a.m., shown a probability that looks confident, is more likely to over-rely on a model trained on 457 clips than a trained nurse would be. The accountability pillar applies directly here: the parent is the only party in the consumer chain, and the system is influencing how they interpret real distress. I would not deploy the consumer version without (a) showing per-class confidence in plain language, including the model's uncertainty, (b) defaulting to ``go check on your baby'' on low-confidence predictions, and (c) refusing to ship any class for which held-out per-class recall does not exceed a published threshold.

\section*{Alternative Resolutions and Risks}
\begin{enumerate}[leftmargin=*,itemsep=1pt,topsep=1pt]
    \item \textbf{Ship as decision support, never as diagnosis.} The model surfaces a probability and a recommended next step, not a clinical label. Risk: users may still over-trust the output (automation bias [5]). Mitigation: UI friction on low-confidence cases.
    \item \textbf{Do not deploy until a clinical trial.} The most cautious option. Risk: opportunity cost; a research artifact never reaches the parents who could benefit. Mitigation: an interim release as an open benchmark with no consumer wrapper.
    \item \textbf{Deploy only the high-recall classes.} Ship \textit{hungry} but withhold \textit{belly\_pain} until its recall passes a published bar. Risk: this removes the clinically valuable signal. Mitigation: an in-app note that pain detection is intentionally not offered until validated.
\end{enumerate}

\section*{Analogous Cases}
The \textbf{Optum healthcare-allocation algorithm} (Obermeyer et al., \textit{Science}, 2019) used spending as a proxy for need and systematically under-referred Black patients. This is the closest precedent for why per-slice metrics are needed in this project, and it motivates the per-class FNR reporting. \textbf{Pulse-oximeter racial bias} (Sjoding et al., \textit{NEJM}, 2020) shows that a sensor whose calibration population was demographically narrow can produce silent harm at scale, which is analogous to a cry corpus collected from one connected demographic. \textbf{IBM Watson for Oncology} was rolled back at MD Anderson for confidently recommending unsafe treatments [4], which supports the decision-support framing and the regulatory pathway. \textbf{COMPAS recidivism} (Angwin et al., \textit{ProPublica}, 2016) shows that an opaque model in a high-stakes pipeline must be auditable per-slice, not just on aggregate accuracy. These cases motivate the design decisions in this project: per-class metric reporting, cross-source robustness audits, decision-support framing, and conservative deployment thresholds.
