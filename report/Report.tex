...
\usepackage{xcolor}
\usepackage{float}
% Tighten float spacing so wide tables / figures do not orphan whitespace
...
\author{
  Syed Omer Shah \quad Collin Murphy \\
  Group \#: 14 \\
...
\end{minipage}
\caption{Confusion matrices on the donateacry test split. Left: baseline (\textit{none + weighted\_ce}, seed 0) -- the model predicts \textit{hungry} for every test instance, including all rare-class cases. Right: best cell (\textit{generative + weighted\_ce}, seed 2) -- the model now distributes predictions, correctly recovers the single test \textit{burping} clip, and partially recovers \textit{discomfort} and \textit{tired}.}\label{fig:confusion}
\end{figure}
...
A deployed cry classifier will not see clean studio spectrograms. To quantify how each augmentation arm degrades under noise, we add zero-mean Gaussian noise of varying $\sigma$ to the standardized log-mel spectrograms of the held-out test set, evaluating at $\mathrm{SNR} \in \{\text{clean}, 20, 10, 5, 0\}$~dB. We compare the no-augmentation baseline (\textit{none + weighted\_ce}) against the best cell (\textit{generative + weighted\_ce}), three seeds each.
...
\toprule
Arm & SNR & macro-F1 & accuracy & belly\_pain rec. & burping rec. & ECE \\
\midrule
baseline (none + weighted\_ce)        & clean & 0.193 & 0.797 & 0.000 & 0.000 & 0.285 \\
...
\midrule
generative + weighted\_ce             & clean & \textbf{0.258} & \textbf{0.817} & 0.000 & \textbf{0.667} & 0.352 \\
                                        & 20 dB & \textbf{0.267} & \textbf{0.831} & 0.000 & \textbf{0.667} & 0.367 \\
                                        & 10 dB & \textbf{0.186} & \textbf{0.768} & 0.000 & 0.333 & 0.334 \\
