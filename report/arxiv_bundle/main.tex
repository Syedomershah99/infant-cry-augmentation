...
% [preprint]: de-anonymizes the author block and removes the "Under review" header.
% For TMLR double-blind submission, drop the option: \usepackage{tmlr}.
...
\author{%
  \name Syed Omer Shah \email syedomer@buffalo.edu \\
  \AND
  \name Collin Murphy \email cjm72@buffalo.edu \\
...
\paragraph{Code and data.} All code, configs, manifests, per-cell artifacts, datasheet, model card, leakage tests, and per-seed bootstrap intervals are publicly released at \url{https://github.com/Syedomershah99/infant-cry-augmentation}. Raw audio is not redistributed; download scripts target the original public sources.
